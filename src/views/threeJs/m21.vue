...
<script setup>
  ...
  onMounted(() => {
    ...
    (width = can.value.offsetWidth), (height = can.value.offsetHeight);
  ...
  function draw() {
    ...
    let lightRotationX = obj.moveY;
    let lightRotationY = -obj.moveX;
    let innerLimit = degToRad(obj.innerlimit);
    let outerLimit = degToRad(obj.outerlimit);
    ...
    let worldViewProjectionLocation = gl.getUniformLocation(program, 'u_viewProjectionMatrix');
    ...
    gl.uniform3fv(envColorLocation, [0.1, 0.1, 0.1]);
    // 设置聚光灯的方向
    let lightDirection;
    let lmat = lookAt(lightPosition, target, up);
